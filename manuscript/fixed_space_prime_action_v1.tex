...
\usepackage{mathrsfs}
\hypersetup{pdftitle={Fixed-Space Prime Compatibility in Burnol's Sonine Quotients -- v1.48},pdfauthor={Alexander Eastwood}}
...
\large Complete working manuscript, version 1.48\\
...
this cost. The unchanged tail metric is false at $\lambda=5$ in both
parities. Source-$L^2$ transport through the fixed point-value repair
...
alternative. At $\lambda=5$, its first four dyadic blocks above
$N=\lambda^2$ already violate the criterion in both parities, even
with the largest admissible block metrics and arbitrary positive row
reweighting. This obstruction concerns the selected block comparison;
...
compactly supported wave-packet falsification attempts.
...
numerical arithmetic stress tests also fail to show decay.  The method
...
Version 1.48 restates the level-count bound with the packet's level mass
normalized against the symbol's own level distribution on the head range
(Proposition~\ref{prop:v148-weighted-levels}). The loss of a $K$-level
minorant is then at least $c'$ times the one-sided $K$-level quantization
cost of that distribution, and the cofinal zero input reduces from a bound
on every level band to the growth of that cost, bounded below through the
concentration function of the level distribution (its density is
unbounded at trough levels, so no sup-density constant exists).
Head diagnostics (NS-28) show the weighted hypothesis is satisfied by
states of negligible energy where the uniform one is not. Both inputs
remain open; nothing is closed. G2 and RH remain open.
...
from about $2.54$ to $5.51$; they are not proof evidence.  The scalar
...
problem, not a consequence of smoothing.  Numerical tests at
$\lambda=3,4,5$ agree with these obstructions and are used only as
falsification diagnostics.  The full signed weighted-superlevel
...
correct, but numerical diagnostics at $\lambda=3,4,5$ show that it
discards most favorable symbol mass. Generic Logvinenko--Sereda
...
positive complete signed energy, including under an independent
physical-space calculation. Thus the scalar shortcuts fail, while the
...
stronger pointwise multiplier criterion is numerically false at already
certified positive fixed windows.
...
bound.  A finite $\lambda=3$, $N=64$ diagnostic falsifies the stronger
pointwise-supersolution shortcut; it neither proves nor disproves the
...
The first four dyadic blocks are individually positive, but certified
coupling lower bounds disprove the criterion for this partition in
both parities. The obstruction survives every admissible smaller
block metric and every positive scalar row reweighting. It is a
failure of that sufficient comparison, not a negative Weil direction,
and supplies no conclusion about other cutoffs or partitions.
...
remains disproved; no complete $\lambda=5$ positivity margin or
...
\subsection*{The unchanged construction fails at $\lambda=5$}
...
would therefore be a modification. The frozen protocol is recorded in
Appendix~\ref{app:v127-transfer}.

\begin{proposition}[A counterexample to the stipulated tail metric]
\label{prop:v127-metric-fails}
Let $D=\operatorname{diag}(a_n)$ be the actual archimedean diagonal
...
\end{proposition}
\begin{proof}
Use the exact coefficient formula and analytic coefficient enclosures
of Proposition~\ref{prop:v114-weil-core}. The outward verifier forms
\[
...
At $320$ and $448$ bits the frozen vectors give the stated strict
...
and reproduces the intervals at $448$ bits. The odd expansion differs
from real-sine coordinates only by a common unit phase. Witness hashes,
coefficient enclosures and the independent verifier are included in
Appendix~\ref{app:v127-transfer}. Each vector has literal finite support,
so its complete quadratic form is exactly this finite sum: no remote
row contributes to the pairing. The ratios are below $10^{-8}$,
proving the assertion on the complete tail.
\end{proof}

The first remote-weight gate already fails at the frozen even
$N=512$ and odd $N=1536$: its certified lower weights are approximately
$-1.0764504091$ and $-1.5489108675$. Fresh evaluation of the same
weight formula gives first positive cutoffs $1502$ and $7224$.
These changes alone cannot repair Proposition~\ref{prop:v127-metric-fails}:
that proposition disproves the required metric itself, independently
of the verification cutoff. More CG steps, greater moment order, or
larger remote support cannot make that fixed inequality true.

The necessary change is to the outer head or the tail metric. Keeping
...
or a resolution margin cannot replace it. These are counterexamples
to the frozen metric, not negative Weil vectors.
...
tests its natural $N=\lambda^2$ instance at $\lambda=5$ and proves
failure on that particular partition. The abstract implication remains
...
\label{sec:v128-test}
The new test fixes $\lambda=5$, $N=\lambda^2=25$ and the literal
parity blocks
\[
...
$Q_{16}$ metric already disproved in Proposition~\ref{prop:v127-metric-fails}.
...
\begin{proposition}[The tested dyadic comparison fails in both parities]
\label{prop:v128-dyadic-fails}
For the four blocks above, each actual signed $T_{jj}$ is strictly
...
\end{proposition}
\begin{proof}
Reconstruct the exact coefficients at $320$ and $448$ bits, with the
same $96$-term analytic archimedean enclosure used above. Frozen
dyadic inverse-Cholesky proposals $C_j$ satisfy the outward
...
pairs $(x_{jk},y_{jk})$. Direct interval evaluation proves positive
...
\[
 ...
 \le\beta_{jk}(T).
\]
The following are exact integer numerators of strict lower bounds
...
Both precision replays use identical frozen witnesses and give
...
to the norm comparison, so these finite obstructions settle failure
of the infinite criterion on this partition. The witness files,
congruence factors and independent check are recorded in
Appendix~\ref{app:v128-test}.
\end{proof}

These numbers are lower bounds for a norm-comparison eigenvalue,
...
inside them, or reweighting scalar rows cannot fix the present test.
...
now rules out its tested $\lambda=5$, $N=25$ dyadic instance, even
after admissible metric changes and scalar row reweighting. Its
...
the saved $\lambda=3$, $N=64$ compression gives a projected ratio with
...
The failure of a global conditional-positive-definiteness shortcut can
be made rigorous at a certified positive physical window.

\begin{proposition}[Global negative index does not model the physical window]
\label{prop:v132-global-index}
At $a=\log4$ the exact symbol in \eqref{eq:v130-beta} satisfies
\[
 -0.64784909<\beta_a(1)<-0.6321938<-\frac35.
\]
Consequently the full-line multiplier by $\beta_a$ has infinite
...
\end{proposition}
\begin{proof}
The displayed enclosure follows from exact rational interval
arithmetic: Machin's formula encloses $\pi$, the atanh series encloses
...
The scalar layer-cake estimate is rigorous but too lossy in the first
tests.  On grids truncated at $|\xi|\le5000$, its already accumulated
...
An adversarial wave-packet test also did not falsify the weighted
mechanism.  For the even packets
...
candidate.  A non-rigorous direct scan of the exact arithmetic symbol
gave $a\Delta(\beta_a)$ approximately $2.5448,3.4002,4.9148,5.5052$
...
The rigid transfer test now disproves the unchanged $10^{-8}D$
tail metric at $\lambda=5$ in both parities
(Proposition~\ref{prop:v127-metric-fails}). Its complete certificate
margin is unavailable, and changing remote cutoffs alone cannot fix
that false prerequisite. A successful head or metric adaptation rule
...
Proposition~\ref{prop:v128-dyadic-fails} now rules out this dyadic
partition under every admissible smaller block metric and every
positive scalar row reweighting. A finite norm-comparison obstruction
already settles that failure; it does not settle the Weil sign.
...
observed alternating symbol train and its relation to the Chebyshev
remainder $\psi(\lambda^2)-\lambda^2$, and the ground's outward spread
with its first nearby zero at $u\simeq\gamma_1L/(2\pi)$, are diagnostic
context for a future signed concentration estimate. They supply no
bound on the source-orthogonal complement and trigger no new experiment
here.
...
The directory \texttt{g2\_scalar\_budget\_no\_go/} contains the proof,
...
\texttt{g2\_radical\_topology/} contains their proof excerpt, candidate and
...
\texttt{g2\_growing\_radical/} contains the proof excerpt, novelty and
...
The bundle directory \texttt{g2\_capacity\_candidate} contains the
candidate register, the complete formula-level novelty comparison and
the adversarial review of Proposition~\ref{prop:v129-capacity}. The
finite projected-source values quoted after that proposition were
recomputed from the archived $\lambda=3$, $N=64$ interval midpoints
only as a falsification diagnostic. They are not interval certificates
and are not used in the proof of the Picone identity.

The review independently expands the archimedean constant, checks both
prime orientation factors, preserves the zero-extension killing terms
and verifies the first-slot-linear pole sign. It also records the
two-coordinate counterexample to the tempting claim that
$W_\lambda h\ge0$ pointwise alone proves positivity. The only rigorous
new implication is therefore the conditional capacity reduction.
The arithmetic capacity estimate, its cofinal error and the odd sector
remain open.

\subsection*{Weighted-concentration and Schatten no-go audit}
The bundle directories \texttt{g2\_weighted\_concentration} and
\texttt{g2\_schatten\_no\_go} contain the proofs, candidate register,
adversarial review and diagnostics for
Propositions~\ref{prop:v131-concentration}--%
\ref{prop:v132-flat-top}. The Hilbert--Schmidt identity and interlacing
bounds are analytic. The dilation countermodel uses the sharp
Dirichlet Poincar\'e inequality and exact unitary scaling; no numerical
concentration eigenvalue is used in its proof.

\texttt{certify\_beta4\_negative.py} uses exact rational interval
arithmetic and proved series remainders. It certifies
$\beta_{\log4}(1)<-3/5$ without a floating-point proof step. The
global negative-index consequence applies to unrestricted Schwartz
tests, not to the compact physical window. The positive flat-top no-go
is likewise analytic and does not assume a particular smoothing
window.

The finite cosine-compression and trapezoidal-flat-top scripts run only
at $\lambda=3,4,5$. They are adversarial diagnostics, not certificates
of the physical form or a cofinal trend. Their purpose is to try to
falsify scalar simplifications before promoting them. The surviving
signed weighted-superlevel inequality remains conditional, and its
cofinal arithmetic constants and the odd sector remain open.

\subsection*{Continuum-cancellation and local no-go audit}
The bundle subdirectory \texttt{g2\_continuum\_cancellation} contains
the exact proof report and the non-rigorous symbol diagnostic for
Propositions~\ref{prop:v130-continuum-cancellation}--%
\ref{prop:v130-local-no-go}. The continuum identity was checked both
as a kernel factorization and by its positive Fourier multiplier.
The six-bump proof is algebraic: its strict support separation, sole
prime--$2$ correlation and two vanishing Laplace moments use no
quadrature or endpoint approximation.

The script \texttt{test\_continuum\_symbol.py} scans only the already
studied windows $\lambda=3,4,5$. It is a falsification diagnostic for
the stronger pointwise criterion, not a sign certificate. The exact
normal form retains the physical support constraint; replacing it by
arbitrary frequency localization would contradict the certified
positive fixed windows and is not done here.
...
\subsection*{Signed-block criterion test and certified failure}
\label{app:v128-test}
The directory \texttt{g2\_block\_metric} contains the protocol frozen
before the test, the original numerical proposals, exact dyadic pair
and congruence witnesses, and outward $320$- and $448$-bit certificates.
The primary cutoff is explicitly $N=\lambda^2=25$ with four blocks
$26{:}50$, $51{:}100$, $101{:}200$, $201{:}400$. The metrics are the
full actual signed diagonal blocks; no scalar tail certificate is
substituted for them.

\nolinkurl{certify_block_failure.py} verifies all four positive blocks
and all six normalized coupling lower bounds in each parity. It
then checks the integer Rayleigh numerators of the comparison
matrices, so no eigensolver accuracy is used in the failure gate.
Every dyadic proposal is reconstructed exactly. The accompanying
independent verifier checks all six block pairings in each parity
using signed Fourier coordinates. The first three blocks alone
already give a comparison Rayleigh value above one in both parities. Both verifiers use the
same audited analytic coefficient enclosures and Arb arithmetic.
This is internal adversarial verification, not a separately derived
analytic kernel or external certification.

The infinite tail is not replaced by zero. A finite positive prefix
could not certify the proposed infinite criterion, but these certified
lower bounds disprove it because additional norm terms cannot decrease
the comparison. The successful gates establish a route obstruction;
there is no complete $\lambda=5$ sign certificate, no generalized
positivity margin, and no new $\lambda=8$ calculation in this run.

\subsection*{Rigid $\lambda=5$ transfer and the stopped certificate}
\label{app:v127-transfer}
The bundle directory \texttt{g2\_lambda5\_transfer} contains the frozen
protocol, both dyadic counterwitnesses, $320$- and $448$-bit replays,
and one obstruction report per parity. These are not positive tail
certificates or inverse factors. The parameter record precedes the
tests and has SHA256
...
stipulated tail prerequisite was disproved.

The counterwitness files have hashes
\begin{quote}\small
...
\end{quote}
\nolinkurl{falsify_fixed_tail_metric.py} replays the complete finite-support
quadratics. \nolinkurl{audit_l5_tail_metric_expanded.py} independently
reassembles them in signed Fourier indices using the same audited
analytic coefficient enclosures and Arb arithmetic. Thus it checks
parity factors and signs, not an independently derived analytic kernel.
...
The cumulative bundle's \texttt{g2\_nested\_commutator} directory
contains the complete independent adverse review,
\texttt{certify\_scaled\_commutator.py}, and its machine-readable
certificate.  The script performs exact rational polynomial integration
for a degree-four Taylor kernel and uses only rational enclosures for
$\pi$ and $\sqrt5$ in its final comparison.  It certifies the stronger
check
\[
 \norm{[C_{1/10},C_{1/5}]}>1.198\cdot10^{-5}.
\]
The main proof uses the shorter degree-two estimate exceeding
$9\cdot10^{-6}$; neither bound is inferred from an eigensolver.  The
passage to the actual source is analytic and depends explicitly on
Lemma~\ref{lem:v115-source-mass}.  No calculation above
$\lambda=5$ was made.  The result is a no-go for an automatic
approximate-commutation simplification, not a negative result for the
arithmetic superlevel hierarchy and not a G2 sign estimate.

\subsection*{Disjoint-channel and primitive-transport reproduction}
The cumulative bundle's \texttt{g2\_primitive\_transport} directory
contains two independent adverse reviews and
\texttt{test\_primitive\_transport.py}.  The disjoint-channel lower
bound in Proposition~\ref{prop:v134-disjoint} is analytic and uses only
$\sin t/t\ge1-t^2/6$ and the proved strong source limit.  No numerical
singular-value estimate enters it.  The primitive-transport review
derives the exact drawdown optimization and the Dirichlet-model value
$\pi^3/6$ independently.  The script evaluates the arithmetic drawdown
on a finite frequency grid for $\lambda=3,4,5,6$; its output is explicitly
diagnostic, not interval-certified and not used in a proof.  No
$\lambda=8$ calculation was made.  The reports also record why direct
Cotlar regrouping is algebraically the existing joint-channel target
