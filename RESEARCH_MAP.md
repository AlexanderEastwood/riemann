...
- [ ] **RH** — via Weil positivity
  - [ ] **Alternative criteria (all equivalent; same wall)** — each is a known RH-equivalent criterion; none is closer; assessed 2026-09-21
    ...
    - [~] **circle / Toeplitz-Hankel lens** — `lane/circle-toeplitz` · evidence: [`evidence/diag_true_symbol/`](evidence/diag_true_symbol/) · Validated symbol beta_a (2e-7 vs block). Ground level split: lambda=3 -/+0.0898, lambda=4 -/+0.2257, cancelling to 1e-38 / 1e-75; ~50% mass on {beta<0}; all energy at xi<20. In lattice units u=xi L/2pi the symbol is a +/- train with crests on the lattice, amplitude set by psi(lambda^2)-lambda^2; the ground spreads outward in u with lambda while the first zeta ordinate sits at the edge of its support. Not self-similar.
