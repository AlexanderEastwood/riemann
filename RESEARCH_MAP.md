...
- [ ] **RH** — via Weil positivity
  ...
  - [ ] **G2: cofinal -o(1)** — `prop:v121-cofinal-rh` · eps_lambda -> 0 cofinally IS RH
    ...
    - [ ] **Uniform mechanism for G2**
      ...
      - [!] **signed weighted concentration** — `prop:v131-concentration` · `lane/concentration` · evidence: [`evidence/v146/`](evidence/v146/) · Valid sufficient criterion (prop:v131). Blocked on the named open inputs of v1.46-v1.49 (QG: exact K-level quantization growth of the symbol's level distribution; an admissible bounded-energy packet with the weighted level hypothesis), not on any diagnostic: the NS-22 finite-head losses are restricted-family upper bounds and its tail charge was not a global minorant (NS-31). Not proved closed.
