"""Certified complete gains for tail-balanced actual divisor feedback."""
...
def run(n: int, kernel: Kernel) -> dict[str, Any]:
    ...
    for power in range(3):
        ...
        physical[-1] = -2*n*sum((physical[k-n-1]/k for k in range(n+1,2*n)),arb(0))
        assert sum((physical[k-n-1]/k for k in range(n+1,2*n+1)),arb(0)).contains(0)
        suffix = arb(0)
        ...
        if power == 0:
            assert all((defect[k]+physical[k-n-1]).contains(0) for k in range(n+1,2*n))
    ...
    for name, vector in zip(['balanced_divisor_cancellation','balanced_linear_defect','balanced_quadratic_defect','diagonal_curvature'],directions+[curvature]):
    ...
    assert 0 < overlap < 1
    endpoint = arb_mat([[arb(k)/(2*n)] for k in range(n+1,2*n+1)])
    extended_basis = arb_mat([[v[i,0] for v in directions]+[endpoint[i,0]] for i in range(n)])
    extended_gram = extended_basis.transpose()*projected*extended_basis
    extended_load = extended_basis.transpose()*g
    assert extended_gram.det() > 0
    extended_gain = (extended_load.transpose()*extended_gram.solve(extended_load,algorithm='precond'))[0,0]
    assert span_gain < extended_gain < full_gain
    quantities = {'energy':energy,'full_relative_gain':full_gain/energy,
                  ...
                  'curvature_energy_fraction_in_span':overlap,'endpoint_augmented_span_fraction':extended_gain/full_gain}
    ...
    return {'N':n,'seconds':round(time.monotonic()-started,3),
            ...
            'gates':{'complete_actual_gram':True,'complete_old_projection':True,'physical_difference_conversion':True,'all_but_last_raw_new_jumps_cancel':True,'raw_tail_coefficient_preserved':True,
...
def main() -> None:
    ...
    report = {'classification':'certified finite tail-balanced divisor feedback and endpoint-augmented span; no cofinal estimate',
