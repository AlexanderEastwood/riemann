% ---------------------------------------------------------------------------
% INSERT FILE: window_comparison_insert.tex   (NS-3, drafted 2026-09-21)
% purpose: place the lambda=4 certificate on the half-width axis used by the
%          published compact-window literature (Zhu; Yoshida / Connes--Consani)
%          and state the CCM mu_lambda positioning.  Source: COMPARISON.md.
% anchor:  manuscript/fixed_space_prime_action_v1.tex, immediately AFTER the
%          paragraph that follows the proof of prop:v126-full-window, i.e.
%          after the line
%            to $L=2\log4$ and are not transferred to other windows.
%          and BEFORE the line
%            \subsection*{The unchanged construction fails at $\lambda=5$}
% cites:   \cite{Zhu2026} (already in the bibliography),
%          \cite{ConnesConsaniMoscovici2025} (already in the bibliography).
% note:    the live tex already uses $\mu_\lambda$ for an inverse-approximation
%          quantity (near prop:v125-simultaneous and eq. at line ~10922); the
%          CCM infimum is therefore written $\mu^{\mathrm{CCM}}_\lambda$ here.
...
$\lambda=4$ certificates are coordinate quantities, not ordinary-norm
eigenvalue bounds, and are not comparable with Zhu's enclosure.
