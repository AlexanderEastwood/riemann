...
| --- | --- | --- |
| v1.25 complete even λ=4 certificate: `g2_simultaneous` | RESTORED | [58 original files](../archive/v1.26_snapshot/g2_simultaneous/), including frozen witnesses, verifiers and saved complete residual ingredients, restored from the original v1.26 bundle. These files also match the original v1.25 bundle. |
| v1.26 complete odd λ=4 certificate: `g2_odd_complement` | RESTORED | [51 original files](../archive/v1.26_snapshot/g2_odd_complement/), including complement and directional witnesses, verifiers, saved reports and the exact shared-head check. |
...
| v1.31–v1.34 concentration evidence and diagnostics | OPEN | `g2_weighted_concentration`, `g2_schatten_no_go`, `g2_nested_commutator` and `g2_primitive_transport` remain absent. Recover their original scripts, saved outputs, candidate/proof reports and adversarial reviews, including `certify_beta4_negative.py`, `certify_scaled_commutator.py` and `test_primitive_transport.py`. The finite diagnostics and the claimed rigorous interval/rational checks must retain their distinct scopes. |
...
See the [recovery record](../github_sync/README_v126_evidence_recovery.md)
and [per-file checksums](../github_sync/v1.26_evidence_recovery.json).
