#!/usr/bin/env python3
"""Shifted infinite-complement inertia certificate at lambda=3.
...
"""
import argparse, json, time, gzip, hashlib, sys
from fractions import Fraction
from pathlib import Path
from flint import arb, arb_mat, ctx
sys.path.insert(0,str(Path(__file__).resolve().parent.parent/"g2_schur_directional"))
...
def ldl_certificate(A):
    ...
    for j in range(size):
        ...
        if pivot>0:positive.append(j)
        elif pivot<0:negative.append(j)
        else:return dict(status='unresolved',index=j,pivot=pivot.str(55)),D
        D.append(pivot)
        for i in range(j+1,size):
            lower[i][j]=(A[i,j]-sum((lower[i][k]*lower[j][k]*D[k] for k in range(j)),arb(0)))/pivot
    return dict(status='inertia_certified',count=size,negative_count=len(negative),
                positive_count=len(positive),negative_indices=negative,
                min_abs_pivot=min((abs(p) for p in D),key=float).str(55)),D

def run(N=256,M=512,J=1024,r=128,parity='even',bits=768,diagonal=False,witness=None,shift="1e-36"):
    start=time.time();L,b,d=assemble_sequences(J,bits)
    assert 1<=N<M<J and r>=1
    aq=Fraction(shift); assert aq>=0
    ashift=arb(aq.numerator)/aq.denominator
    d=[x-ashift for x in d]
    ...
    report=dict(status='CERTIFIED INERTIA OF A WHOLE-OPERATOR LOWER FORM' if result['status']=='inertia_certified'
                else 'UNRESOLVED: interval LDL contains a zero pivot',
                exact_shift=str(aq),shift_requested=shift,
                ...
                scope='Inertia of a lower Schur form bounds the negative index of W-shift. It is not equality without a matching trial-space bound. Fixed window only; G2 and RH open.',
                elapsed_seconds=time.time()-start)
    suffix='_diagonal' if diagonal else ''
    shift_tag=shift.replace('/','_over_')
    path=BASE/f'shift_{shift_tag}_N{N}_M{M}_J{J}_r{r}_{parity}_b{bits}{suffix}.json'
...
if __name__=='__main__':
    ...
    p.add_argument('--shift',default='1e-36')
