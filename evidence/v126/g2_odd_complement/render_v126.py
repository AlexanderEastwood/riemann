...
B=Path(__file__).resolve().parent;r=B/'renders_v126';r.mkdir(exist_ok=True)
...
for i,page in enumerate(doc):
 ...
 if i+1 in [1] or any(term in page.get_text() for term in ['One repaired subspace', 'Complete odd complement', 'Complete positivity at', '0.429140', 'Odd complement and complete', 'remaining target is complete ordinary']):page.get_pixmap(matrix=fitz.Matrix(1.5,1.5),alpha=False).save(str(r/f'page_{i+1:03d}.png'))
