...
> What stands: the symbol `β_a` is the right object with the `(−1)^n` basis sign (independent mpmath quadrature confirms even and odd entries at λ=4; the "no odd tail" of §10 means omitted as `O(log X/X³)`, not zero); the deep-block cancellation pattern at finite head in both parities (counts below 1e-8/1e-16/1e-30 survive a 120/180-digit replay; the N=96 values of order −1e-116 at λ ≥ 6 were arithmetic-floor artefacts and are positive at 180 digits); the close alignment of the ground pencil direction with the certified λ=3 source (overlap 0.99999998, not 1; the √2 factor comes from combining the ±n complex modes, not from an unnormalized constant mode; nothing is measured at other windows).
> What is withdrawn: (§5) "no λ-independent limiting picture" and "self-similar argument has nothing to act on" — four shapes differ, which is all that was measured; (§6) "the lobe deepens like a" — a measured sequence 0.67/0.55/1.44/2.13, not a local asymptotic; (§7) the blanket "six relative digits however small" — the correct first-order statement is `δν/ν = −v*(δW⁻)v` under relative *form* control of `W⁻`, which entry spot checks and the two-entry tail script do not supply; `W` in the solve is a midpoint at dps−5, so nothing below the arithmetic floor is meaningful; "the block is converged at λ=4" (12 → 13 from N=48 to 96); (§8) `η_a^op → ∞` and "the unweighted layer-cake form does not give a uniform floor" as conclusions — the ratios 0.36/0.35/0.34/0.34 are four full-head, ξ ≤ 800 observations whose inequality direction against the source-compressed complete η is not established; (§10) "the same count" (6/4/1 vs 5/3/1; 12/10/7 vs 12/9/6) and "three to four decades" (matched N=48: 3.6 decades; the 1e-75 comparison mixed cutoffs); throughout, "nothing beyond", "no mass", "all energy at ξ<20", "c = 0" mean below printed resolution, not exact.
