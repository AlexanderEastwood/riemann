...
combination, the prime main terms cancel exactly after the old normal
equations; the signed divisor forcing remains. The complete numerator is
positive at N=16 and 256, but separating its terms by absolute values gives
a vacuous lower bound. At N=256 that bound is below -186 times the current
error, and below -226 after doubling the physical cutoff. These finite
checks diagnose an estimate; they do not disprove the direction.

For fixed N with a nonzero residual logarithmic or constant tail, the
absolute divisor allowance diverges as the cutoff grows, although the
signed expression converges. This is not a uniform statement at T=N^6.
The necessary cofinal signed lower estimate remains open. Manuscript stays
v1.66; no new error-decay theorem or finite-efficiency result is claimed.

[Attempt and complete evidence](evidence/ns90_mobius_gain/) ·
[Readable report](audits/mobius-lower-gain-2026-09-23-v1.html).
...
orthogonal improvement increments. This gives a complete next-size upper
certificate and the cumulative envelope `E_(2^(J+1)) <= K exp(-sqrt(2)*S_J)`,
where `S_J` sums `abs(d_(2^j))/E_(2^(j-1))` from a fixed starting index.
A positive logarithmic lower growth of this scalar arithmetic sum would
imply a sufficient logarithmic whole-error upper rate. That growth is
unproved; the envelope alone supplies no decay theorem or fast solver.

The exact identity also excludes an eventual numerator lower bound
`abs(d_(2^j))/E_(2^j) >= a/j^p` for `0<=p<1`; for `p=1`, `a>1/sqrt(2)`
is excluded by NS83. This closes the specified stronger estimate only.
Finite N=8,16,32,64 defects change sign, and gain capture dips to 0.3256%
at N=16. No eventual lower bound is inferred. The cumulative finite
envelope is looser than the existing directly optimized finite bounds.
Two precisions, full Gram/physical cutoff replays and Maxima checks pass.
Manuscript v1.66, RH/G2 and the missing arithmetic lower growth stay unchanged.

[Readable report](audits/dilation-upper-envelope-2026-09-23-v1.html) ·
[Proof, complete checks and Maxima](evidence/ns88_dilation_defect/).
