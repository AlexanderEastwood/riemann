...
def sequences(lam,J,bits=768,K=96):
 ctx.prec=bits;path=B/f'sequences_v3_l{lam}_J{J}_b{bits}_k{K}.json'
 if path.exists():
...
def block(rows,cols,parity,b,d,a=None,c=0):
 def e(n,m):
  if n==m==0:return d[0]-(c*a[0] if a is not None else 0)
  if n==0 or m==0:
   assert parity=='even'
   return arb(2).sqrt()*b[n+m]/(n+m)
