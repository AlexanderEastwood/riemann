...
| NS-93 | Test a concrete averaged arithmetic estimate for the selected cost and gain, retaining all signed correlations and projection losses | Codex | claimed | Derive and audit an actual arithmetic estimate or record its precise remaining input; do not count the known cumulative-gain criterion as a new result. |
| NS-94 | Check the tangent-circle inversion and exterior similarity-point proposal in ordinary coordinates and Maxima | Codex | claimed | Read the supplied Power of a Point source, distinguish geometric identities from zero-count assumptions, and give a plain-language explanation with an exact off-line symmetric control. |
