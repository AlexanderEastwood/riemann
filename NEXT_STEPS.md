...
| NS-103 | Diagnostic: window at which the plain Weil form restricted to support [-lambda,lambda] detects Davenport-Heilbronn off-line zeros; zeta form as pipeline check | Claude | done (diagnostic; no version); interpretation clarified by NS104 | evidence/diag_ns103_window_calibration/: four observed first-grid detections at logarithmic half-widths1.9/2.3/2.2/2.4; an offset-only guess fails. Finite float bases and thresholds, not proved minimum windows or a universal height law. The bandwidth count is heuristic. NS104 corrects the manuscript parameter conversion and limits the near-null explanation to a tested finite-trial pattern. Original numerical artifacts retained; no G2/RH claim. |
...
| NS-104 | Audit Claude’s ground-mechanism prediction using the unchanged archived even trial and v1.43 bounds; distinguish log-window conventions, finite trial and complete ground | Codex | done (diagnostic/audit; PR57 pre-registered before claim; no version) | evidence/diag_ns104_ground_mechanism/: all64 trial endpoint sign changes at1024/1280 bits; first linearized shift -3.4231e-38, about465 times below sqrt(rho)/slope. Complete later-ground roots and null-space mechanism unproved. NS103 half-width convention corrected; no hard height cutoff inferred. v136 needs a common absolute floor, not relative resolution of every near-null energy. No thaw, new window or cofinal claim. |
| NS-105 | Diagnostic: spectral profile of the archived normalized even lambda=4 trial between and beyond the zeta ordinates (envelope of the transform, sign changes vs N(T), crossover ratio); pre-registered in draft PR #59 | Claude | done (diagnostic; no version) | evidence/diag_ns105_trial_spectrum/: 8000-point Arb grid at 1024/1280 bits, identical results. Envelope of the trial transform decays 0.2-0.28 decades per unit height from 20 to 160 and plateaus at 0.04-0.5 sqrt(rho) above 200; sign changes match N(T) one for one within 0.05 of every ordinate up to 150 (29 below the 32 pi crossover), degrade 150-200 where the envelope meets the plateau, and decouple above. P2/P3 confirmed, P1 shape confirmed but its 1e-30 threshold missed (crossed at 118.8). Finite trial only; no complete-ground transfer, version, G2 or RH claim |
