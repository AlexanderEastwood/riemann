...
| NS-103 | Diagnostic: window at which the plain Weil form restricted to support [-lambda,lambda] detects the Davenport-Heilbronn off-line zero (delta=0.3085, T=85.7); zeta form as positivity check | Claude | done (diagnostic; no version) | evidence/diag_ns103_window_calibration/: PROPOSAL.md filled per gate; explicit formula validated against zeros for zeta and for the Davenport-Heilbronn function; min eigenvalue vs lambda for both; result: an off-line zero at height T is first detected at window lambda ~ (1/2)log(T/2pi)+0.7 nearly independent of its offset (four Davenport-Heilbronn zeros, offsets 0.31/0.22/0.15/0.07 detected at 1.9/2.3/2.2/2.4); negative energy ~ offset^2; the 1/(2 delta) guess is refuted; zeta form nonnegative throughout with a Paley-Wiener near-null space reaching a fraction of height 2pi e^{2 lambda}. Floats; not a certificate; no G2/RH claim |
...
| NS-104 | Audit Claude’s ground-mechanism prediction using the unchanged archived even trial and v1.43 bounds; distinguish log-window conventions, finite trial and complete ground | Codex | in progress (diagnostic/audit; PR57 pre-registered before claim) | First64 zero evaluations only; no new window, eigenfunction reconstruction, cofinal claim or thaw. Proposal in PR57 and evidence/diag_ns104_ground_mechanism/PROPOSAL.md. |
