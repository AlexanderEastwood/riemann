...
| NS-103 | Diagnostic: window at which the plain Weil form restricted to support [-lambda,lambda] detects the Davenport-Heilbronn off-line zero (delta=0.3085, T=85.7); zeta form as positivity check | Claude | claimed | evidence/diag_ns103_window_calibration/: PROPOSAL.md filled per gate; explicit formula validated against zeros for zeta and for the Davenport-Heilbronn function; min eigenvalue vs lambda for both; lambda* and resonant frequency reported. Floats; not a certificate; no G2/RH claim |
